...
- **The peer wire is now HTTP** (`GET /pkg/<name-version>`), the seed half exists (`daemon.CacheSource` + `peer.Server`), `internal/peerwire` is gone, and the size bound is the exact `pkgsize`/`cksum` pair from the repository database rather than a constant. What remains of §5.3 is mounting the seeder on `serving_addr` — see HANDOFF §5.4.
