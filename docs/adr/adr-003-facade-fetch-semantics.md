...
**Not decided here:** whether the facade should also *cache* what it proxies. It plainly could — the bytes pass through it — but the daemon has no store by design and `AGENTS.md` says the pkg cache is read-only, so a store is a separate decision with its own constraints. Seeding continues to come from the pkg cache, which pkg populates from what the facade serves it, so the swarm is fed either way.
